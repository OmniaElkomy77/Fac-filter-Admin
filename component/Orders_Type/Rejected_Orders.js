...
export default class Rejected_Orders extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            RejectedOrder_arr: [],
    ...
    async RejectedOrder_fun() {
        let data_to_send = "start"
        let fetch = await axios.post(Api.Domain + "select_refused_orders.php", data_to_send);
        if (fetch.status == 200) {
            let res = fetch.data

            if (res.status == 'success') {
                this.setState({
                    RejectedOrder_arr: res.message, pageLoading: false,
            ...
            <>
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <Text style={{ fontSize: 22, color: "#fff", fontWeight: "bold" }}>الطلبات  المرفوضه</Text>
                    </View>
                </View>
            </>
        )
        ...
        return (
            <>
                <View style={{ marginBottom: 100 }}>
                    ...
                    <FlatList
                        keyExtractor={item => `wcp22#-${item}`}
                        data={this.state.RejectedOrder_arr}
                            ...
                            <View
                                ...
                                <View
                                    ...
                                    <Text
                                        ...
                                        هل ترغب في جعل الطلب قيد التنفيذ؟{' '}
                                    </Text>
                                </View>

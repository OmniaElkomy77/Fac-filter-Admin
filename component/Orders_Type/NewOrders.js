...
export default class NewOrder extends React.Component {
    ...
    async NewOrder_fun() {
        ...
        let fetch = await axios.post(Api.Domain + "select_accepted_orders.php", data_to_send);
        if (fetch.status == 200) {
            let res = fetch.data

            if (res.status == 'success') {
                this.setState({
                    NewOrder_arr: res.message, pageLoading: false,
            ...
            <>
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <Text style={{ fontSize: 22, color: "#fff", fontWeight: "bold" }}>الطلبات  الجديده </Text>
                    </View>
                </View>
            </>
        )
    }
    _renderBodyof_newOrder() {
